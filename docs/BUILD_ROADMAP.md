...
> **Status (2026-07-17): Part B largely EXECUTED.** The Employee Master Database shipped — `createEmployee`/`updateEmployee`/`archiveEmployee` Cloud Functions (`functions/src/hr/employees/`), employee domain types, real List/Profile/Form pages, rules — starting at commit 26d3dcf. Deliberate deviations from the plan below: the compensation sub-collection + `updateEmployeeCompensation` were **deferred** (the schema sits in a clearly-marked PLANNED section of `src/types/employee.types.ts`); Vitest TDD tasks were **skipped** (no test runner in the repo — `npm run build` + dev-server verification is the gate, per CLAUDE.md); Create/Edit pages merged into one `EmployeeFormPage.tsx`; `STOREKEEPER`/`CUSTOMER_SERVICE` roles were dropped from `ROLES`. Checkboxes below are left as the historical plan record.
...
**Tech Stack:** React 18 + TypeScript + Vite + React Router, Tailwind CSS + shadcn-style primitives, Zustand, Firebase (Auth, Firestore, Cloud Functions v2, Storage). (React Hook Form + Zod and Vitest + React Testing Library appear in the task bodies below but are NOT installed — aspirational at time of writing; the shipped build used plain state + manual validation and no test runner.)
...
- **Resolved — UI library:** `docs/core/ARCHITECTURE.md`, `DESIGN.md`, and `COMPONENT_LIBRARY.md` used to specify Material UI while the shipped code used Tailwind + shadcn-style primitives. The docs were corrected (July 2026) to match the code — no conflict remains.
