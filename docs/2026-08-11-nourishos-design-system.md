...
| 1.2 | 2026-09-25 | **Scoped exception for `welcome/` only** — the New-Hire Welcome Portal (`docs/modules/welcome-portal.md` §9). That app uses gradients, glassmorphism and page/step choreography, over a palette extended with the brand decks' deep teal `#1D4244`, amber `#FFCC80` and olive on top of Pandan `#0E4F47`. Rationale: a new hire sees those screens a handful of times, not fifty per shift, so the rationing rule §2 and §8 impose on the internal app buys nothing there. **Recorded so it is not read as drift, and it applies nowhere else** — no internal-app surface gains glass or gradients from this. Held unchanged even inside `welcome/`: WCAG AA over every glass state (glass carrying text is tinted ≥55% opacity), `prefers-reduced-motion` replacing all movement with opacity fades, `prefers-reduced-transparency` and missing `backdrop-filter` falling back to solid tints, 16px inputs, 48px touch targets, status never by colour alone, Archivo + IBM Plex Mono only, and Indonesian-length strings (no fixed-width buttons, no truncated primary labels). Performance guardrails for mid-range Android: blur 12–16px, at most three blurred layers per viewport, `transform`/`opacity` only, drift paused when the tab is hidden. Tokens live in `welcome/src/glass.css` and are never hardcoded in components. **The one departure that is a behaviour change, not a visual one**: `welcome/` blocks submit while offline instead of honouring D3's offline queue — a hire fills the form in once, at home, and a queued submit that silently fires hours later is worse than a clear "you are offline"; the draft is cached either way. |
