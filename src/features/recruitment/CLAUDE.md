...
## Deleting rejected candidates (2026-09-08)

`deleteCandidate` is superAdmin/HR Manager only (`recruitment.delete`, granted to `hrManager`; superAdmin bypasses as usual) and only accepts a candidate at `ST-07` (Rejected) — `CandidateDetailPage.tsx`'s trash icon only renders in that state. It hard-deletes the candidate doc plus its `confidential/application` sub-document, `discResults/{candidateId}`, and any `interviews` scheduled against it, in one batch. Uploaded candidate documents (`files/{id}`) are deliberately not touched — that's the shared file-storage engine's own `deleteFile` callable, not wired in here.

`CandidatePipelinePage.tsx`'s Rejected column also got a "delete all" control next to its count badge — no new backend endpoint, it's a client-side loop calling `deleteCandidate` once per card in that column (the callable's own per-candidate ST-07 + permission check is what actually enforces it; the loop just drives it N times and reports how many failed).
