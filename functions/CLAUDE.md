...
`functions/tools/sync-role-permissions.mjs` is a hand-run Admin SDK script that reconciles live `roles/{roleId}` documents against `ROLE_PERMISSIONS`. It exists because that map only seeds a role document the **first time** the role is claimed, so every permission string added by every module since then never reaches documents already in Firestore — the callable then enforces a string nobody holds and the feature is invisible with no error to point at. Every module has hit this; the per-module `CLAUDE.md` files record it as a caveat, and this is the automation. Dry run by default, `--apply` to write, `--prefix shiftReports.` to scope it to one module during a rollout. **It only ever adds** — permissions on a live document but absent from the map are reported and left alone, since they may have been granted deliberately through Settings > Roles & Permissions. `superAdmin` is skipped (absent from the map by design, and it bypasses `requirePermission` anyway). Reads the compiled map, so build first.
