 ...
 * in-app only (the WhatsApp leg, §9.7's actual delivery channel, is blocked
 * on a Fonnte account this environment doesn't have — see CLAUDE.md).
 *
...
async function notifyForThreshold(
  ...
  for (const doc of snap.docs) {
    ...
    try {
      await notifyUsersByRole({ role: 'hrManager', module: 'hr', title, message, referenceId: doc.id, priority })

      if (daysAhead <= 30) {
        const leaderRole = DEPARTMENT_ROLES[employee.departmentId as string]?.[0]
        if (leaderRole && leaderRole !== 'hrManager') {
          await notifyUsersByRole({ role: leaderRole, module: 'hr', title, message, referenceId: doc.id, priority })
        }
      }
      if (daysAhead <= 7) {
        await notifyUsersByRole({ role: 'generalManager', module: 'hr', title, message, referenceId: doc.id, priority: 'high' })
      }
    } catch (error) {
