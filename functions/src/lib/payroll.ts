 ...
 *
 * Hand-mirrored subset of src/constants/payroll.ts. functions/ is a separate
 * npm package with its own tsconfig and cannot import from src/, the same
 * intentional duplication as collections.ts / permissions.ts / positions.ts.
 * Change one, change both.
 *
 * Decision 2 (§2): statutory components live in code because their identity,
 * rate keys and bases are law, not configuration; the discretionary catalogue
 * lives in Firestore (`payrollComponents`), seeded from PAYROLL_COMPONENT_SEEDS.
 */
