...
describe('evaluateGate (§6.1)', () => {
  test('a binding with no gate is open immediately', () => {
    assert.equal(
  ...
  test('an unmet prerequisite keeps it locked', () => {
  ...
  test('every prerequisite completed opens it', () => {
    const binding = { ...openBinding, prerequisiteTopicIds: ['trn-a', 'trn-b'] }
    assert.equal(
  ...
  test('a tenure gate holds until the month count is reached', () => {
    const binding = { ...openBinding, minTenureMonths: 3 }
    const args = { binding, completedTopicIds: [], deptOnboardingTopicIds: [] }
    assert.equal(evaluateGate({ ...args, tenureMonths: 2 }), false)
    assert.equal(evaluateGate({ ...args, tenureMonths: 3 }), true)
  })

  test('allCoreTopics needs every onboarding topic in the department, not just the prerequisites', () => {
  ...
  test('a Set and an array of completed ids behave identically', () => {
    const binding = { ...openBinding, prerequisiteTopicIds: ['trn-a'] }
    const args = { binding, tenureMonths: 0, deptOnboardingTopicIds: [] }
    assert.equal(evaluateGate({ ...args, completedTopicIds: new Set(['trn-a']) }), true)
    assert.equal(evaluateGate({ ...args, completedTopicIds: ['trn-a'] }), true)
  })
})

describe('department mapping (§D3)', () => {
  test('the app taxonomy maps onto the sheet taxonomy', () => {
    assert.equal(resolveTrainingDepartment('bar', 'nourish_uluwatu'), 'dept-bar')
    assert.equal(resolveTrainingDepartment('fb_service', 'nourish_berawa'), 'dept-fandb-service')
    assert.equal(resolveTrainingDepartment('wholefood_retail', 'wholefood_ungasan'), 'dept-wholesale')
  })

  test('kitchen resolves by outlet — restaurant kitchen vs bakery production', () => {
    assert.equal(resolveTrainingDepartment('kitchen', 'nourish_ungasan'), 'dept-kitchen')
    assert.equal(resolveTrainingDepartment('kitchen', 'the_bakery_kitchen'), 'dept-bakery-kitchen')
  })

  test('departments with no set in the sheet map to null rather than borrowing one', () => {
    for (const departmentId of ['admin_general', 'sales_marketing', 'housekeeping']) {
      assert.equal(resolveTrainingDepartment(departmentId, 'boh_nourish_group'), null)
    }
    assert.equal(resolveTrainingDepartment('not_a_department', 'nourish_uluwatu'), null)
  })

  test('every mapped target exists in the seeded departments', () => {
  ...
  test('every seeded department is reachable from some app department', () => {
...
describe('tenure and due dates', () => {
  test('tenure counts whole months and does not credit the part-month', () => {
    assert.equal(tenureMonthsSince('2026-01-15', '2026-04-14'), 2)
    assert.equal(tenureMonthsSince('2026-01-15', '2026-04-15'), 3)
    assert.equal(tenureMonthsSince('2025-08-31', '2026-08-31'), 12)
  })

  test('a missing or malformed hire date is 0 months, never negative or NaN', () => {
    assert.equal(tenureMonthsSince(undefined), 0)
    assert.equal(tenureMonthsSince('not-a-date'), 0)
    assert.equal(tenureMonthsSince('2027-01-01', '2026-01-01'), 0)
  })

  test('manual bindings get no due date — §3, they never surface as overdue', () => {
    assert.equal(dueDateFor('manual', '2026-08-26'), null)
    assert.equal(dueDateFor('none', '2026-08-26'), '2026-09-25')
    assert.equal(dueDateFor('interval', '2026-12-20'), '2027-01-19')
  })

  test('assignment ids are deterministic per employee and canonical topic', () => {
    assert.equal(assignmentId('emp1', 'trn-a'), 'emp1__trn-a')
  })
})

describe('seed data integrity (§3)', () => {
  test('the ingestion counts are what the spec claims', () => {
    assert.equal(seed.departments.length, 11)
    assert.equal(seed.trainingTopics.length, 197)
    assert.equal(seed.trainingBindings.length, 217)
  })

  test('every binding resolves to a real topic and a real department', () => {
    ...
    for (const binding of seed.trainingBindings) {
      ...
      assert.ok(
        departmentIds.has(binding.departmentId),
        `${binding.id} points at missing department ${binding.departmentId}`,
      )
    }
  })

  test('every prerequisite resolves — an unresolvable one is a topic nobody could ever unlock', () => {
    const topicIds = new Set(seed.trainingTopics.map((topic) => topic.id))
    for (const binding of seed.trainingBindings) {
  ...
  test('no binding lists itself as its own prerequisite', () => {
    for (const binding of seed.trainingBindings) {
      assert.ok(!binding.prerequisiteTopicIds.includes(binding.topicId), `${binding.id} depends on itself`)
    }
  })

  test('a prerequisite is always bound to the same department, earlier in the sequence', () => {
  ...
  test('ids are unique in all three collections', () => {
    for (const [name, rows] of Object.entries(seed)) {
      assert.equal(new Set(rows.map((row) => row.id)).size, rows.length, `${name} has duplicate ids`)
    }
  })

  test('phase and recurrence use only the values the model allows', () => {
  ...
  test('a new Bar hire opens exactly the topics with no gate (§6.1 expected shape)', () => {
    ...
    assert.ok(
      open.every(
        (binding) => binding.prerequisiteTopicIds.length === 0 && !binding.allCoreTopics && !binding.minTenureMonths,
      ),
