...
NourishOS is an internal operational platform for Nourish Group Indonesia, a multi-outlet F&B company — a React SPA frontend backed by **Firebase** (Auth, Firestore, Cloud Functions, Storage). It is in early milestone development. Implemented today: **HR appraisal** (including AI insights), **HR employee database**, **HR recruitment**, **HR Inventory** (uniform & asset stock), **HR Employment Contracts** (versioned history plus GM→Director signing), **HR Training** (catalog, assignment, completion), **HR Offboarding & Exit Interviews** (exit checklist, clearance statement, F009 exit interview survey), **HR Reports**, **Security patrol/checkpoints**, **Operations** (Lost & Found, Incident Reports, Daily Updates, Work Orders, Checklists, Projects), **Documents** (Job Descriptions, SOP Library), **Communications** (Announcements, Tasks, Team Chat), **Finance** (Expense Requests), the **executive Calendar** (with Google Calendar sync), **Reports** (GM Flash Report), **Settings** (Roles & Permissions, Communication Settings), and the **dashboard** — frontend pages plus their Cloud Functions. **There are no `<ModulePlaceholder>` route stubs left** — `/reports` was the last one and is now the GM Flash Report page (the component still exists, unused by any route). **There is no top-level Purchasing, Inventory, CRM, or Departments module** — Purchasing/Inventory/CRM were cut from scope (2026-08-15); Departments > People shipped 2026-08-16 and was deleted again the same day (see "Current state of the tree"). HR Inventory at `/hr/inventory` is the only inventory concept in the app.
