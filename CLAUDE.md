...
- **The Documents module ships one sub-module: Job Descriptions (2026-08-14).** `/documents` is no longer a `<ModulePlaceholder>` — its index is the register ([src/features/documents/jobDescriptions/](src/features/documents/jobDescriptions/)), with `job-descriptions/new`, `job-descriptions/access`, and `job-descriptions/:jobDescriptionId/edit` beneath it, backed by four callables in [functions/src/documents/jobDescriptions.ts](functions/src/documents/jobDescriptions.ts). Content is curated by hand by the super admin, so the shipped state is an empty register. Several firsts live here: **`systemSettings/jobDescriptionAccess` is the first runtime-editable config doc** (an allowlist of role ids written by `setJobDescriptionAccess`) and the reason `firestore.rules` now contains **its first cross-document `get()`** — module read access can't ride on a custom claim if it's editable at runtime. It is matched by literal doc id so the sibling `*NumberSequences` counters stay in the deny-all, and before the doc exists the `get()` yields null and only `isElevated()` roles can read (closed by default, on purpose). **`requireSuperAdmin` ([functions/src/lib/rbac.ts](functions/src/lib/rbac.ts)) is the first role-based server gate** — every other callable checks permission strings, but `superAdmin` is absent from `ROLE_PERMISSIONS` and its role doc was hand-seeded, so a new `jobDescriptions.manage` string would 403 on a fresh install. Two deviations: `docs/modules/documents.md` §4/§20 has no Job Descriptions section or collection, and the PDF is an **external URL on the doc** rather than a `files` record queried by `resourceType`/`resourceId` (the PDFs already live in the company's Drive; nothing is uploaded to Storage). The link is `new URL()`-parsed server-side and restricted to http(s) because it renders into an `<a href>`.
